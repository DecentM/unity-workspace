...
namespace DecentM.Chat
{
    ...
    public class ProfilePictureStore : UdonSharpBehaviour
    {
        ...
        private void Start()
        {
            ...
            if (this.store == null) this.store = new RenderTexture[0];
            ...
            this.cameraObject.transform.SetPositionAndRotation(this.cameraTemplate.transform.position, this.cameraTemplate.transform.rotation);
        ...
        {
            if (playerId < 0 || playerId > this.store.Length - 1) return null;
        ...
        public void TakePicture(int playerId)
        {
            ...
            this.cameraObject.transform.SetPositionAndRotation(head.position + (head.rotation * Vector3.forward * 2), head.rotation);
