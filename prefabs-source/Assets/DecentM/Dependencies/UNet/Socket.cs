...
namespace UNet
{
    ...
    public class Socket : UdonSharpBehaviour
    {
        ...
        {
            if (connection != null)
                connection.RequestSerialization();
        }
            ...
            for (int i = 0; i < sendMessages_count; i++)
            {
                ...
                if ((messagesToSend & bit) != 0)
                {
                    ...
                    if (
                        ...
                        && (sendMessages_values_expectedMask[index] &= mask) == 0ul
                    )
                    {
                        if (i == 0)
                            shiftMessages = true;
                        ...
                        manager.OnSendComplete(
                            this,
                            (messageStartId + i) % IDS_COUNT,
        ...
        public void CancelSend(int id)
        {
            ...
            else
            {
                Debug.LogErrorFormat(
                    "Unable to cancel sending because message with ID {0} has already been sent",
                    id
                );
            }
        }
        ...
        public int SendAll(bool sequenced, byte[] data, int count)
        {
            ...
            sendBufferReference[0] = (byte)(
                (sequenced ? TYPE_SEQUENCED : TYPE_NORMAL) | TARGET_ALL
            );
            return result;
        }
        ...
        public int SendMaster(bool sequenced, byte[] data, int count)
        {
            ...
            sendBufferReference[0] = (byte)(
                (sequenced ? TYPE_SEQUENCED : TYPE_NORMAL) | TARGET_MASTER
            );
            return result;
        }
        ...
        public int SendTarget(bool sequenced, byte[] data, int count, int targetConnection)
        {
            ...
            sendBufferReference[0] = (byte)(
                (sequenced ? TYPE_SEQUENCED : TYPE_NORMAL) | TARGET_SINGLE
            );
        ...
        {
            int result = TryAddMessage(
                sequenced,
                connectionsMask,
                1 + connectionsMaskBytesCount,
                data,
                count
            );
            if (result < 0)
            ...
            sendBufferReference[0] = (byte)(
                (sequenced ? TYPE_SEQUENCED : TYPE_NORMAL) | TARGET_MULTIPLE
            );
        ...
        private int TryAddMessage(
            bool sequenced,
            ulong targets,
            int headerSize,
            byte[] data,
            int dataSize /*, out byte[] buffer*/
        )
        {
            if (sendMessages_count >= BUFFER_SIZE)
                return -1;
            ...
            {
                Debug.LogErrorFormat(
                    "Message is too long: {0}, max size: {1}",
                    fullMsgSize,
                    MAX_MESSAGE_SIZE
                );
                return -1;
        ...
        {
            if (IdDiff(idStart, messageStartId) > 0)
                mask <<= (idStart - messageStartId + IDS_COUNT) % IDS_COUNT;
            else
                mask >>= (messageStartId - idStart + IDS_COUNT) % IDS_COUNT;
        ...
        public int PrepareSendStream()
        {
            ...
            if (sendAttemptsMask == messagesToSend)
                sendAttemptsMask = 0;
        ...
        public void OnReceiveSequenced(
            int connectionIndex,
            int id,
            int sequence,
            byte[] dataBuffer,
        ...
        {
            if (id < IDS_COUNT_HALF)
                id += IDS_COUNT;
            if (reference < IDS_COUNT_HALF)
                reference += IDS_COUNT;
            return id - reference;
        }
		#endregion
