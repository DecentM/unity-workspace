...
{
    public class SrtParser : Parser<SrtLexer, SrtToken>
    {
        ...
        public override Ast Parse(List<SrtLexer.Token> tokens)
        {
            ...
            string ConsumeUntil(params SrtToken[] type)
            {
                int tCursor = cursor;
            ...
            while (cursor < tokens.Count)
            {
                ...
                if (mode == Mode.ExpectingScreenIndex)
                {
                    ...
                    // token type 2 == int
                    if (current.type != SrtToken.Number)
                    {
                        cursor++;
                        continue;
                    }
                ...
                if (mode == Mode.ExpectingStartTimestamp)
                {
                    ...
                    string timestamp = ConsumeUntil(SrtToken.Space);
                ...
                if (mode == Mode.ExpectingArrow)
                {
                    ...
                    if (current.type != SrtToken.Arrow)
                    {
                        cursor++;
                        continue;
                    }
                ...
                if (mode == Mode.ExpectingEndTimestamp)
                {
                    ...
                    string timestamp = ConsumeUntil(SrtToken.Space, SrtToken.Newline);
                    int timestampMillis = this.ParseTimestamp(timestamp, ',', ':');
                ...
                if (mode == Mode.ExpectingTextContent)
                {
                    ...
                    if (current.type == SrtToken.Newline)
                    {
                        cursor++;
                        continue;
                    }
                    ...
                    while (consecutiveNewlines < 2)
                    {
                        ...
                        // tokenType 7 == newline
                        if (tCurrent.type == SrtToken.Newline)
                        {
                            consecutiveNewlines++;
                        }
                        else
