...
{
    public class VttParser : Parser<VttLexer, VttToken>
    {
        private enum Mode
        {
            ExpectingHeader,
        ...
        public override Ast Parse(List<VttLexer.Token> tokens)
        {
            ...
            string ConsumeUntil(params VttToken[] type)
            {
                int tCursor = cursor;
            ...
            while (cursor < tokens.Count)
            {
                ...
                // TMPro styles.
                if (current.type == VttToken.Style)
                {
                    cursor++;
                    continue;
                }
                ...
                {
                    if (current.type != VttToken.WEBVTTHeader)
                    {
                        cursor++;
                        continue;
                    }
                ...
                if (mode == Mode.ExpectingStartTimestamp)
                {
                    ...
                    // token type 2 == int
                    if (current.type != VttToken.Number)
                    {
                        cursor++;
                        continue;
                    }
                    ...
                    // index
                    if ((cursor + 1) < tokens.Count && tokens[cursor + 1].type == VttToken.Newline)
                    {
                        cursor++;
                        continue;
                    }

                    string timestamp = ConsumeUntil(VttToken.Space);
                ...
                if (mode == Mode.ExpectingArrow)
                {
                    ...
                    if (current.type != VttToken.Arrow)
                    {
                        cursor++;
                        continue;
                    }
                ...
                if (mode == Mode.ExpectingEndTimestamp)
                {
                    ...
                    string timestamp = ConsumeUntil(VttToken.Newline, VttToken.Space);
                    int timestampMillis = this.ParseTimestamp(timestamp, '.', ':');
                ...
                {
                    if (current.type == VttToken.Newline)
                    {
                        cursor++;
                    ...
                    while (cursor < tokens.Count && tokens[cursor].type != VttToken.Newline)
                    {
                ...
                if (mode == Mode.ExpectingTextContent)
                {
                    ...
                    while (
                        ...
                        && tokens.ElementAt(tCursor).type != VttToken.DoubleNewline
                    )
                    {
                        VttLexer.Token tCurrent = tokens.ElementAt(tCursor);
